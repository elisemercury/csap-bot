...
class TeamsBot(Flask):
    ...
    def setup_webhook(self, name, targeturl, wh_resource, wh_event,
        ...
        for webhook in whlist:
            wh = self.teams.webhooks.delete(webhookId=webhook.id)
    ...
    def process_incoming_message(self):
        ...
        if post_data["resource"] != "messages":
            ...
            for c in sorted(self.commands.items()):
                # Need to remove the Bot's Display Name from the text before searching 
                # TODO: Remove all mentionedPeople 
                if self.extract_message(self.teams.people.me().displayName, message.text).lower().find(c[0]) != -1:
                    command = c[0]
    ...
    def extract_message(self, command, text):
        ...
        # If command not found.. return full string.
        if cmd_loc == -1: 
            message = text 
        else: 
